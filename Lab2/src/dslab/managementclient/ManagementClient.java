...
public class ManagementClient {
    ...
    private static BillingServerInterface billingServer;
    private static BillingServerSecureInterface secure;
	...
	if (args.length == 2) {
		...
		try {
			...
			billingServer = (BillingServerInterface) registry.lookup(args[1]);
			secure = null;
			String user = null;
			...
			AnalyticsCallbackInterface callback = (AnalyticsCallbackInterface) registry.lookup(args[0]);
			...
			while ((in = stdIn.readLine()) != null) {
				...
				if (input[0].equals("!login")) {
					...
					System.out.println("Wrong command: !unsubscribe requires exactly one additional argument! Usage: !unsubscribe subscriptionID");
				    }
					...
					System.out.println("Wrong command: !auto requires no additional argument! Usage: !auto");
				    }
				    ...
				    System.out.println("Command not recognized! Please use either !login, !logout, !addStep, !removeStep or !bill, !subscribe, !unsubscribe, !auto, !hide or !print !");
				}
			    } else {
			...
			System.out.println("BillingServer or AnalyticsServer not bound");
		    } catch (IOException e) {
		    ...
		    //free all ressources
		    //as we are shutting down we don't really care about the exception's messages
